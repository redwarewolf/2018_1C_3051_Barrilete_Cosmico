...
float3 hsv2rgb(float3 c)
{
	...
	float3 p = abs((c.yyy + K.xyz - floor(c.yyy + K.xyz)) * 6.0 - K.www);
	return c.z * (K.xxx*(1- c.x) + clamp(p - K.xxx, 0.0, 1.0)* c.x);
}

//Pixel Shader
float4 ps_main(PS_DIFFUSE_MAP input) : COLOR0
{
	//float4 ret = tex2D(diffuseMap, input.Texcoord) * input.Color * (color_global);

	float pos = input.Texcoord.x;
	float3 col = float3(1.0, (time * speed) + pos, 1.0);

	return (tex2D(diffuseMap, input.Texcoord) * input.Color * (color_global)) * float4(hsv2rgb(col), 1);
...
//{
//	float pos = input.Texcoord.x;
//	float3 col = float3((time * speed) + pos, 1.0, 1.0);
//	//float alpha = tex2D(diffuseMap, input.Texcoord).a;
//
//	return v_vColour * float4(hsv2rgb(col), 1);
//}
